...
uint64 
sys_take_shared_memory_request(void) {
  struct proc *p = myproc();
  if (crypto_srv_proc == 0 || p != crypto_srv_proc) {
      printf("Invalid server process\n");
      return -1;
  }
  ...
  // Only release src_proc lock if dst_va == 0, as map_shared_pages already releases locks
  if (dst_va == 0) {
  ...
  if (copyout(p->pagetable, arg_dst_va, (char*)&dst_va, sizeof(dst_va)) < 0) {
    printf("Failed to copy out dst_va\n");
    return -1;
  }
  if (copyout(p->pagetable, arg_dst_size, (char*)&req.size, sizeof(req.size)) < 0) {
    printf("Failed to copy out size\n");
    return -1;
  }
